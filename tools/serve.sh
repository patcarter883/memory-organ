...
docker run --rm --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -v "$MINISGL":/minisgl:ro -v "$ENGINE":/engine:ro -v "$ENGINE/cam_ckpt":/ckpt -v "$DATA":/data:ro -v "$CACHE":/probe_cache \
  ...
  "source /app/.venv/bin/activate && timeout ${PYTIMEOUT:-1500} python /engine/cam/recall_mag.py \
     ...
     --serve --export-serving /ckpt --persistent-cohort 10 2>&1" | tee -a "$LOG" \

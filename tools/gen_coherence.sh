...
# passes ${CNAME:+--name} to docker and prints "[gen] done" on success.
set -uo pipefail
...
LOG=/home/pat/code/memory-organ-p/tools/gen_coherence.out
echo "[gen] start $(date -u +%H:%M:%S)" | tee "$LOG"
docker run --rm ${CNAME:+--name "$CNAME"} --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_GEN_INJECT_STEPS="${INJ_STEPS:-2}" -e CAM_GEN_LEN="${GEN_LEN:-16}" -e CAM_GEN_SAMPLE="${GEN_SAMPLE:-12}" \
  ...
  "source /app/.venv/bin/activate && timeout 2400 python /engine/cam/recall_mag.py \
  ...
  | grep -E 'edits across|binding held-out|GENERATION COHERENCE|OFF:|ON :|edit |NEW object|Traceback|Error'
echo "[gen] done $(date -u +%H:%M:%S)" | tee -a "$LOG"

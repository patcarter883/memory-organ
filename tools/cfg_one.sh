#!/usr/bin/env bash
# args: SEG QA TAG  — one legacy N=137 triad run at a given seg-len/qa-seg. Runs inside the gpu-lease shell
# (so $HIP_VISIBLE_DEVICES / $ROCR_VISIBLE_DEVICES are set).
set -uo pipefail
SEG=$1; QA=$2; TAG=$3
...
LOG=/home/pat/code/memory-organ-p/tools/cfg_${TAG}.out
echo "[cfg] === seg=$SEG qa=$QA ($TAG) $(date -u +%H:%M:%S) ===" | tee "$LOG"
docker run --rm --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_WRITE_AT_READ=1 -e CAM_TRIAD_DEBUG=1 -e CAM_LOGIT_INJECT_SWEEP="0,2,8,20" -e CAM_LOCALITY_NBR_CAP=1 \
  -e CAM_COHORT_CAP=80 -e CAM_PROMPT_MAXTOK=48 -e CAM_CONF_DIAG=1 \
  ...
  "source /app/.venv/bin/activate && timeout 2700 python /engine/cam/recall_mag.py \
     ...
     --seg-len $SEG --qa-seg $QA --save-anyway --conf-gate --locality-weight 0 \
     --persistent-locality --persistent-cohort 10 2>&1" | tee -a "$LOG" \
  | grep -E 'edits across|binding held-out|cf-delivery|edits below C0|AssertionError|Error'
echo "[cfg] done $TAG $(date -u +%H:%M:%S)"

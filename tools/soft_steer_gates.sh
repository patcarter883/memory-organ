#!/usr/bin/env bash
# Track 5 (#99): TWO-GATE ablation on the SURGICAL path. All configs: CAM_BIND_TRUE=1 (reinforce true),
# CAM_SOFT_STEER=1 (logit-only, no blunt tap), alpha=2, store-conf Gate A on. We toggle Gate B (base<->store
# AGREEMENT, CAM_AGREE_GATE): 0 = store-conf only, 1 = store-conf AND agreement. Prediction: Gate B zeroes the
# ΔP(true) HARM in the confident bucket (base already right -> inert) while preserving the lifts in the
# uncertain/wrong buckets (base disagrees -> still pushed). One lease, two container runs.
set -uo pipefail
...
LOG="$ENGINE/tools/soft_steer_gates.out"
echo "[gates] start $(date -u +%H:%M:%S)" | tee "$LOG"

run_cfg () {  # $1 = CAM_AGREE_GATE, $2 = label
  echo "[gates] === agree_gate=$1 ($2) ===" | tee -a "$LOG"
  docker run --rm --device /dev/kfd --device /dev/dri --group-add video \
    ...
    -e CAM_SOFT_STEER=1 -e CAM_LOGIT_INJECT="${ALPHA:-2}" -e CAM_LOGIT_GATE_C0="${GATE_C0:-1}" -e CAM_LOGIT_GATE_HARD=1 \
    -e CAM_AGREE_GATE="$1" \
    ...
    | grep -E 'Track 5|bucket|\[0\.|\[1\.|ALL |LEANS|Traceback|Error'
}

run_cfg 0 storeconf-only
run_cfg 1 storeconf-AND-agree
echo "[gates] done $(date -u +%H:%M:%S)" | tee -a "$LOG"

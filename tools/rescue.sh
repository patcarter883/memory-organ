...
docker run --rm --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_WRITE_AT_READ=1 -e CAM_BIND_TRUE=0 -e CAM_SOFT_STEER=1 -e CAM_LOGIT_GATE_C0=1 -e CAM_LOGIT_GATE_HARD=1 \
  -e CAM_RESCUE_ALPHA="${RESCUE_ALPHA:-0.5}" -e CAM_RESCUE_TAU="${RESCUE_TAU:-0.5}" -e CAM_MULTIGATE_TOPK=16 \
  ...
  "source /app/.venv/bin/activate && timeout ${PYTIMEOUT:-1500} python /engine/cam/recall_mag.py \
     ...
     --persistent-rescue --persistent-cohort 10 2>&1" | tee -a "$LOG" \
  | grep -E 'PROVENANCE|confident-|uncertain|CONFIDENT-WRONG|Traceback|Error'

...
docker run --rm --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_LOGIT_INJECT_SWEEP="${ALPHA_SWEEP:-0,2,8,20}" -e CAM_LOCALITY_NBR_CAP="${NBR_CAP:-3}" \
  -e CAM_COHORT_CAP="${COHORT_CAP:-0}" -e CAM_PROMPT_MAXTOK="${PROMPT_MAXTOK:-64}" -e CAM_CONF_DIAG="${CONF_DIAG:-0}" \
  -e CAM_BANK_SWEEP="${BANK_SWEEP:-}" -e CAM_VALUE_UNIT_NORM="${VALUE_UNIT_NORM:-0}" \
  -e CAM_WRITE_AT_READ="${WRITE_AT_READ:-1}" -e CAM_TRIAD_DEBUG="${TRIAD_DEBUG:-0}" \
  ...
  "source /app/.venv/bin/activate && timeout ${PYTIMEOUT:-1200} python /engine/cam/recall_mag.py \

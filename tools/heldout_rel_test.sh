...
# Watchdog-compatible: passes ${CNAME:+--name} to docker and prints "[heldoutrel] done" on success.
set -uo pipefail
...
LOG=/home/pat/code/memory-organ-p/tools/heldout_rel_test.out
echo "[heldoutrel] start $(date -u +%H:%M:%S)" | tee "$LOG"
docker run --rm ${CNAME:+--name "$CNAME"} --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_WRITE_AT_READ=1 -e CAM_HELDOUT_RELATIONS="${HELDOUT_RELS:-2}" \
  -e CAM_CONF_DIAG=1 -e CAM_TRIAD_DEBUG=1 -e CAM_LOGIT_INJECT_SWEEP="0,2,8,20" \
  -e CAM_LOCALITY_NBR_CAP=0 -e CAM_COHORT_CAP=80 \
...
echo "[heldoutrel] done $(date -u +%H:%M:%S)" | tee -a "$LOG"

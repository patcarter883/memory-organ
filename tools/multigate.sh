#!/usr/bin/env bash
# Track 5 (#99): THE MULTIGATE. Composed delivery at the injection site, all label-free signals:
#   δ = g_storeconf · g_headroom(1-p_base(target)) · g_scope(store decode peak) · sparse_topk · α·out_proj(value)
# Neutralises all three documented failure modes at once — leak (storeconf, §3.15), confident-fact harm
# (headroom, §3.24), novel-relation corruption (scope veto, §3.23) — and the SPARSE support collapses the
# blunt collateral KL (§3.24) so the lean is gentle by construction. alpha=1. Two configs, one lease:
#   (A) ungated push (storeconf only)  -> the raw lean at alpha=1 (big KL)
#   (B) FULL MULTIGATE                 -> preserve uncertain lift, inert confident bucket, LOW KL
set -uo pipefail
...
LOG="$ENGINE/tools/multigate.out"
echo "[mg] start $(date -u +%H:%M:%S)" | tee "$LOG"

run_cfg () {  # $1 = CAM_MULTIGATE, $2 = label
  echo "[mg] === multigate=$1 ($2) ===" | tee -a "$LOG"
...
run_cfg 0 ungated-alpha1
run_cfg 1 FULL-MULTIGATE
echo "[mg] done $(date -u +%H:%M:%S)" | tee -a "$LOG"

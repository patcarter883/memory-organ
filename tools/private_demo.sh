...
docker run --rm ${CNAME:+--name "$CNAME"} --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_MT_RECON_NAME_TOKENS="${MT_NAME:-1}" -e CAM_MT_VALUE_NO_NORM="${VALNONORM:-0}" \
  -e CAM_RECON_MAGNITUDE="${RECON_MAG:-0}" \
  ...
  "source /app/.venv/bin/activate && timeout 600 python /engine/cam/recall_mag.py \
     ...
     --perpos-key ${PERPOS_KEY:-codebook} --readout ${READOUT:-linear} --mt-positions 4 --mt-recon-weight 1.0 \

#!/usr/bin/env bash
# Track 5 (#99): SOFT-MARGIN gate at a GENTLER alpha. The hard argmax agree-gate failed (base already
# argmaxes true at low prob -> it suppressed the whole lean). Correct gate = push ∝ HEADROOM on the target
# (1 - p_base(target)): full where the base gives the target nothing, inert where it's already certain.
# alpha=1 (gentler than the alpha=2 near-override). Two configs, one lease:
#   (A) storeconf only            -> the ungated lean at alpha=1
#   (B) storeconf + MARGIN gate   -> should PRESERVE the uncertain-bucket lift, drive the confident bucket
#                                    to ~0, and LOWER collateral KL (self-dosing).
set -uo pipefail
...
LOG="$ENGINE/tools/soft_steer_margin.out"
echo "[margin] start $(date -u +%H:%M:%S)" | tee "$LOG"

run_cfg () {  # $1 = CAM_MARGIN_GATE, $2 = label
  echo "[margin] === margin_gate=$1 ($2) ===" | tee -a "$LOG"
  docker run --rm --device /dev/kfd --device /dev/dri --group-add video \
    ...
    -e CAM_SOFT_STEER=1 -e CAM_LOGIT_INJECT="${ALPHA:-1}" -e CAM_LOGIT_GATE_C0="${GATE_C0:-1}" -e CAM_LOGIT_GATE_HARD=1 \
    -e CAM_MARGIN_GATE="$1" \
    ...
    | grep -E 'Track 5|P\(true\)_off|\[0\.|\[1\.|ALL |Traceback|Error'
}

run_cfg 0 storeconf-only
run_cfg 1 storeconf-AND-margin
echo "[margin] done $(date -u +%H:%M:%S)" | tee -a "$LOG"

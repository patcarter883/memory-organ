...
docker run --rm ${CNAME:+--name "$CNAME"} --device /dev/kfd --device /dev/dri --group-add video \
  ...
  -e CAM_POOLED_SUBJ_KEY=1 -e CAM_SUBJ_ONLY_QUERY=1 -e CAM_LEARNED_KEY_POOL=1 -e CAM_DISJOINT_BANKS="${BANKS:-512}" \
  ...
  -e CAM_MT_VALUE_NO_NORM="$VALNONORM" -e CAM_GATE_CALIB="${CALIB:-0}" -e CAM_CALIB_OUT_N="${OUTN:-24}" -e CAM_BANK_DTYPE="${BANKDT:-fp32}" -e CAM_MT_DECODE_DIAG="${MTDIAG:-0}" \

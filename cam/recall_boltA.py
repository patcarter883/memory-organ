...
Run:
  python -m cam.recall_boltA --steps 3000
"""
import argparse
import math
import os
...
def eval_direct(model, builder, rng, args, n=2048):
    """Held-out DIRECT acc: does the injected prefix encode the answer (tied-unembed argmax)? carry vs
    ablated. Multi-token (direct_logits -> [B,Kc,vocab], ans [B,Kc]): the PRIMARY carry is EXACT-MATCH
    (all Kc answer tokens correct); per-token acc is also returned. Single-token: both equal the 0/1 hit
    (byte-identical to before). Returns (carry_exactmatch, ablated_exactmatch, carry_pertok, abl_pertok)
    — callers that unpack only the first two get the headline carry/ablated unchanged."""
    model.eval()
    em = {"carry": 0.0, "ablated": 0.0}
    pt = {"carry": 0.0, "ablated": 0.0}
    seen = 0
    while seen < n:
        cur = min(args.batch, n - seen)
        ids, ans, apos = builder.build(rng, cur, local=False)
        ids, ans = ids.to(DEV), ans.to(DEV)
        _dec = getattr(model, "readout", "linear") == "decoder"
        for cond in ("carry", "ablated"):
            pref = model.inject(ids, args.seg_len, builder.qa_start, apos, carry=(cond == "carry"))
            # decoder readout is autoregressive + teacher-forced (gold ans ids); linear/bolt take prefix only
            logits = model.direct_logits(pref, ans) if _dec else model.direct_logits(pref)
            pred = logits.argmax(-1)
            if logits.dim() == 3:                       # [B,Kc,vocab] multi-token
                hit = (pred == ans).float()             # [B,Kc]
                em[cond] += hit.min(dim=1).values.sum().item()   # exact-match: all Kc correct
                pt[cond] += hit.mean(dim=1).sum().item()          # per-token fraction
            else:
                h = (pred == ans).float()
                em[cond] += h.sum().item(); pt[cond] += h.sum().item()
        seen += cur
    model.train()
    return em["carry"] / seen, em["ablated"] / seen, pt["carry"] / seen, pt["ablated"] / seen
...
def main():
    ...
    d_carry, d_abl, _pt_c, _pt_a = eval_direct(model, builder, rng, args)
